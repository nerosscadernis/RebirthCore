...
namespace Rebirth.World.Datas.Maps
{
    public class MapPoint : ICloneable
    {
        ...
        public MapPoint(CellMap cell)
        {
            this.m_cellId = cell.Point.CellId;
            this.SetFromCellId();
        }
        ...
        {
            Point left = new Point((point.X > this.m_x) ? 1 : ((point.X < this.m_x) ? -1 : 0), (point.Y > this.m_y) ? 1 : ((point.Y < this.m_y) ? -1 : 0));
        ...
        public static MapPoint GetPoint(CellMap cell)
        {
            return MapPoint.GetPoint(cell);
        }
